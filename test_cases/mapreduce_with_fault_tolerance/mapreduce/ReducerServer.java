...
public class ReducerServer extends UnicastRemoteObject implements ReducerService{
	...
	public int masterLaunchReducer(List<String> intermediate_location, String appClassPath, String outputFilePath) throws RemoteException {
		...
		Map<String, List<String>> after_sort = new HashMap<String, List<String>>();
			// to call shuffle and sorts on intermediate files contents
		after_sort = shuffleAndSort(intermediate_files);
	...
	private static Map<String, List<String>> shuffleAndSort(List<List<Map<?,?>>> intermediate_file) {
		/*
		...
		for (List<Map<?,?>> list : intermediate_file) {
			for (Map<?,?> pair: list) {
	...
	public static void main(String[] args) throws RemoteException{
		try {
			...
			Registry registry = LocateRegistry.createRegistry(port);		//registry with port number
